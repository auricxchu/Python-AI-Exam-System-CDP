...
- **学生列表排序**：成绩管理学生表格支持按姓名、学号、得分、提交时间排序，默认学号升序，表头带深浅主题适配
- **成绩单文件名优化**：改为 `学号_姓名_ExamReport.txt` 格式，批量下载后按学号自然排序
- **成绩单删除**：支持单条删除和批量删除成绩单，同步清理数据库记录和云端存储文件
...
- **成绩管理 Tab**：TeacherDashboard 新增「题库管理 / 成绩管理」顶层导航，教师无需离开应用即可查看所有考试提交
- **考试场次分组**：成绩按考试名称自动分组，每场显示学生数、平均分、时间范围
- **成绩预览**：Modal 弹窗展示完整成绩单，包含能力完成度进度条、评估依据、轻量扣分标签及证据、代码对比（CodeDiffViewer）、AI 参考答案
- **批量下载**：支持多选学生 → ZIP 打包下载，引入 jszip 依赖
...
- **更新弹窗简化**：点击更新后弹窗锁定不可关闭，X 按钮自动隐藏，下载完成前必须等待
- **toast 图标统一**：已是最新版本 toast 与后台同步 toast 图标样式完全一致（白色圆形背景 + CheckCircle）

## v1.2.8 (2026-05-27)

### 修复

- **macOS 重启安装修复**：quitAndInstall 后增加 3 秒兜底 relaunch + exit，解决无签名应用的更新重启问题
